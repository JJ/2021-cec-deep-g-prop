...
% We are using an expression since we are optimizing three things at the same time, although to be honest, I think that the fitness does have the bigger importance (although F2 will also have some influence) - JJ
%
% \begin{figure}
%     \centering
%     \caption{
...
\begin{table*}
\centering
<<spambase.table,echo=FALSE, results=tex>>=
...
@
\caption{Baseline error percentage results for {\spambase} per partition, with validation (used by the algorith) and test (unseen data, used for generalization) results.}
