...
{\sf EvoMLP} algorithms regarding accuracy for % G-Prop are the results on top? - JJ
...
Optimizing the hidden layers of networks classifying the \emph{cancer1} dataset; 200 evaluations, 7 neurons in the hidden layer (top), without hidden layer optimization (middle), using only hidden layer (bottom)} % What is optimized and what's not? The number of layers? Or the initial weights? - JJ
