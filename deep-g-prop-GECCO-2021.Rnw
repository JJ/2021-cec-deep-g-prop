...
\item Loss function: We selected the cross-entropy loss function to fit the
  ...
  because it provides a fast generalization method. As opposed to
  G-Prop, which used MSE.
...
In this section, results shown are the mean of five runs with different
