...
generations for the \emph{cancer1} dataset.}
     ...
     \begin{tabular}{rllll}
         ...
         \hline
         \textbf{Changing}  & \textbf{Validation} & $0.57$                 &        $32$             &      $0.98765$ \\
                        & \textbf{Test}       & $2.86$                 &        $32$             &      $0.96014$ \\
         \textbf{Fixed}  & \textbf{Validation} & $0.00$                 &        $32$             &      $1.00000$ \\
...
\emph{cancer2} dataset, it seems that the algorithm's accuracy increases when
we mutate the weights of the hidden layer. See \autoref{tab:fixed-7-1-noconst-can2}.
...
mutating the hidden layer weights for the \emph{cancer2} dataset (top) or
leaving them unchanged (bottom).
     }
...
% Changing (hidden layer weights and biases) is the same as mutable (mutable can also mean changing the number of neurons) - lulivi
%
...
This opens up a whole range of possible modifications to try in the research of
better results escaping from static and non-changing solutions. We will see how they compare with G-Prop in the next section.
