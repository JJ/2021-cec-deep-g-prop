...
the search space; this was further limited by the evolution of only single-layer perceptrons. The fact that it was mostly programmed ad-hoc limited its evolution. The unavailibility of modern tools for publication and release of open source software also limited its utility.
...
\cite{ecer2020training,Gupta2020,10.1007/978-981-15-7130-5_38}; however, recent papers like the one by
...
We will first need to adapt the different parts of the original G-Prop, so that we can advance further. We will use a canonical evolutionary algorithm, as implemented by the DEAP evolutionary algorithm library.
...
% Are we sure about the lexicographical comparison? Is it not using a single expression? - JJ
%
% \begin{figure}
%     \centering
%     \caption{
...
distinguishing two very similar individuals. This metric has the ability to
...
predictions.
% We need to expand this a little bit with what's written below - JJ
...
\cite{uci} repository. In this section, results shown are the mean of five runs with different
