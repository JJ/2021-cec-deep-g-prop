...
\begin{itemize}
  ...
  \item An individual is coded as a list of \emph{Layers}. Each layer is
    ...
    \cite{py-numpy} ndarrays) joined toguether with it's configuration (a
    ...
    All the weights and bias are randomly initialized wiht a uniform
    distribution between -1.0 and 1.0. A random proportion of all this genes
    and biases are muted for all the indiivduals each generation.
  ...
  \item For each generation we clone the best half individuals of the current
    ...
    operation has it own probability to occur chosen when running DeepGProp
    CLI. For the weights/bias mutation, a percentage of each (selected too with
    the CLI parametters) are muted randomly. After that, the worst half is
