...
original G-Prop. \begin{itemize}
    ...
    % I think the representation or gene structure is missing, 
    % It is a vector? what values (domain) can have?
    % How the vector becomes a ANN? 
  ...
  \item There are several kinds of {\em mutation} operators: (initial) weights,
    ...
    layers. The neuron will be added last in the randomly selected layer, % number of neurons? -M
    and it will be eliminated in the same way. Since the actual position of the
    neurons is arbitrary, it does not really matter where the neuron % Maybe add: because it is a fully connected network -M
...
the Equation \ref{eq:hidden-score}:
 % This is important because is related to many principles of ML
...
% This is the first time the name DeepGProp appears, is better if this is presented in the
...
\item Activation function: We selected a (\emph{rectified linear unit}) ReLU 
      activation function for hidden layers and Softmax for the output
      layer. % WE need to justify that. - JJ
\item Optimization algorithm: We used the Stochastic Gradient Descend 
      (SGD) backpropagation algorithm with a learning rate value of
      \texttt{0.01}.
      % We have probably said that before, but we need to refer back
      % to where, and also justify - JJ
\item Loss function: We selected the cross-entropy loss function. % Again, justify choice, and also compare it with G-Prop - JJ
\end{itemize}
...
hidden layers from the model. We can appreciate an improvement from the
...
loose function reaches $\approx0.6$. Then the best result obtained is arround
...
from 2 to 50 neurons per layer (choosed randomly).
...
%        múltiples capas variables.
        Comparison of the results obtained for the \emph{smapbase} partitions
...
In this paper a first approach to perform deep neural networks evolution (evolution of MLPs with several layers) has been proposed. 
In our approach, the architecture (i.e. the initial weights, number of hidden layers and hidden layer sizes) of multilayer perceptrons with multiple layers is automatically designed using an EA, taking advantage of the EA to find a solution close to the global optimum and the ability of the training algorithm to tune the solution and reach the nearest local minimum.

Results of the evolution of networks with two layers have been reported, thus demonstrating that the method can be applied to the evolution of networks with a high number of layers.

As future works, we propose using deep neural nets with any number of layers to solve complex problems.
